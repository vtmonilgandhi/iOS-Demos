...
open class CocoaMQTT: NSObject, CocoaMQTTClient, CocoaMQTTFrameBufferProtocol {
    ...
    public func buffer(_: CocoaMQTTFrameBuffer, sendPublishFrame frame: CocoaMQTTFramePublish) {
...
extension CocoaMQTT: GCDAsyncSocketDelegate {
    ...
    public func socket(_: GCDAsyncSocket, didReceive trust: SecTrust, completionHandler: @escaping (Bool) -> Swift.Void) {
    ...
    public func socketDidSecure(_: GCDAsyncSocket) {
    ...
    public func socket(_: GCDAsyncSocket, didRead data: Data, withTag tag: Int) {
    ...
    public func socketDidDisconnect(_: GCDAsyncSocket, withError err: Error?) {
        socket.delegate = nil
        ...
        DispatchQueue.main.async {
            ...
            if !self.disconnectExpectedly && self.autoReconnect && self.autoReconnectTimeInterval > 0 {
                self.autoReconnTimer = Timer.every(Double(self.autoReconnectTimeInterval).seconds, { [weak self] (_: Timer) in
...
extension CocoaMQTT: CocoaMQTTReaderDelegate {
    func didReceiveConnAck(_: CocoaMQTTReader, connack: UInt8) {
    ...
    func didReceivePublish(_: CocoaMQTTReader, message: CocoaMQTTMessage, id: UInt16) {
    ...
    func didReceivePubAck(_: CocoaMQTTReader, msgid: UInt16) {
    ...
    func didReceivePubRec(_: CocoaMQTTReader, msgid: UInt16) {
        printDebug("PUBREC Received: \(msgid)")

        puback(CocoaMQTTFrameType.pubrel, msgid: msgid)
    }

    func didReceivePubRel(_: CocoaMQTTReader, msgid: UInt16) {
        printDebug("PUBREL Received: \(msgid)")

        puback(CocoaMQTTFrameType.pubcomp, msgid: msgid)
    }

    func didReceivePubComp(_: CocoaMQTTReader, msgid: UInt16) {
    ...
    func didReceiveSubAck(_: CocoaMQTTReader, msgid: UInt16) {
    ...
    func didReceiveUnsubAck(_: CocoaMQTTReader, msgid: UInt16) {
    ...
    func didReceivePong(_: CocoaMQTTReader) {
